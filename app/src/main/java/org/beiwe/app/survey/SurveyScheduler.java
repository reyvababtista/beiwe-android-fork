...
public class SurveyScheduler {
	...
	/* This function is only called in the survey downloader, this logic IGNORES the scheduling
	/* logic.  Instead, create the notification immetiately (via broadcast, which sets app state. */
	...
	public static void scheduleSurvey (String surveyId) {
		...
		// turns out the first day of the week is not necessarily Sunday. So, in case the user is in
		// such a Locale we manually set that.
		thisDay.setFirstDayOfWeek(Calendar.SUNDAY);
		today = thisDay.get(Calendar.DAY_OF_WEEK) - 1;
		
		try {
			JSONTimes = new JSONArray(timeString);
		} catch (JSONException e) {
			// If this fails we have significant problems, but probably the errors come from
			// external factors.
			e.printStackTrace();
		...
		for (int i = 0; i <= 6; i++) {
			try {
				dayJSON = new JSONArray(reorderedDays.get(i));
			}  //convert to json array
			catch (JSONException e) {
				e.printStackTrace(); //Again, if this crashes we have problems but they probably come from external factors.
				throw new NullPointerException(e.getMessage());
			}
		...
		// we now have a double nested list of lists.  element 0 is today, element 1 is tomorrow the
		// inner list contains the times of day at which the survey should trigger, these times are
		//   values between 0 and 86,400.
		// these values should be sorted.
